...
rewardmatrix = np.array([[0, 0, 0], [0, 10, 0], [0, 0, 0]])
def expected_value(currentmatrix, i, j, newi, newj, gamma):
...
def bellman(currentmatrix, i, j, gamma):
    neighbours = get_neighbours(currentmatrix, i, j)
    values = []
    for neighbour in neighbours:
        values.append(expected_value(currentmatrix, i, j, neighbour[0], neighbour[1], gamma))
    return max(values)


def bellman_policy(currentmatrix, i, j, gamma):
    neighbours = get_neighbours(currentmatrix, i, j)
    values = []
    for neighbour in neighbours:
        values.append(expected_value(currentmatrix, i, j, neighbour[0], neighbour[1], gamma))
...
def repeated_bellman(currentmatrix, gamma, epsilon, iterations):

    nextmatrix = np.zeros((3, 3))

    for i in range(currentmatrix.shape[0]):
        for j in range(currentmatrix.shape[1]):
            nextmatrix[i][j] = bellman(currentmatrix, i, j, gamma)
    ...
    if np.all(np.abs(nextmatrix - currentmatrix) < epsilon): # if the difference is smaller than epsilon
        ...
        for i in range(currentmatrix.shape[0]):
            for j in range(currentmatrix.shape[1]):
                #π(s)=argmax_a ∑_s′ p(s′|s,a)*(r(a,s,s′) + γ*V_k−1[s′])
                print("Policy for state " + str((i, j)) + " is: " + str(bellman_policy(nextmatrix, i, j, gamma)))
    ...
    return repeated_bellman(nextmatrix, gamma, epsilon, iterations)
...
matrix = (repeated_bellman(empty, 0.9, 0.0000000001, 0))
